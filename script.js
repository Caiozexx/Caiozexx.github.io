...
function gerarCartelaHTML() {
    ...
    for (let linha = 0; linha < 5; linha++) {
        ...
        for (let coluna = 0; coluna < 5; coluna++) {
            ...
            // Adicione margem para espaçamento entre números
...
function playBingo() {
    ...
    if (vetorJogadores.every(jogador => jogador.cartela === undefined)) {
        alert("Você precisa gerar pelo menos uma cartela antes de iniciar o jogo!!!");
        return;
    }

    jogoRolando = true;
    let botaoGerarCartela = document.getElementById("botaoGerarCartela");
    botaoGerarCartela.disabled = true;
...
function reiniciarJogo() {
    console.log("Iniciando reinício do jogo...");
